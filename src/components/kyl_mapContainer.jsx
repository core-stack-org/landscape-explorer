...
const MapLegend = ({ showMWS, showVillages, currentLayer }) => {
    ...
    const isTerrainLayerActive = currentLayer?.some(layer => 
        layer.name === "terrainCluster_ID" || 
        layer.name.includes("terrain")
    );

    const isRainfallLayerActive = currentLayer?.some(layer => 
        layer.name === "avg_precipitation" || 
        layer.name.includes("mws_layers")
    );

    const isDroughtLayerActive = currentLayer?.some(layer => 
        layer.name === "drought_category" || 
        layer.name.includes("cropping_drought")
    );

    const isRunoffLayerActive = currentLayer?.some(layer => 
        layer.name === "avg_runoff" || 
        layer.name.includes("mws_layers")
    );

    const isDryspellLayerActive = currentLayer?.some(layer => 
        layer.name === "avg_number_dry_spell" || 
        layer.name.includes("cropping_drought")
    );

    const isPopulationLayerActive = currentLayer?.some(layer => 
        layer.name === "total_population" || 
        layer.name.includes("panchayat_boundaries")
    );

    const isSTPopulationLayerActive = currentLayer?.some(layer => 
        layer.name === "percent_st_population" || 
        layer.name.includes("panchayat_boundaries")
    );

    const isSCPopulationLayerActive = currentLayer?.some(layer => 
        layer.name === "percent_sc_population" || 
        layer.name.includes("panchayat_boundaries")
    );

    const isLiteracyLayerActive = currentLayer?.some(layer => 
        layer.name === "literacy_level" || 
        layer.name.includes("panchayat_boundaries")
    );

    const isWaterLayerActive = currentLayer?.some(layer => 
        layer.name === "avg_rabi_surface_water_mws" ||
        layer.name === "avg_zaid_surface_water_mws" ||
        layer.name.includes("water_bodies")
    );

    const isNREGALayerActive = currentLayer?.some(layer => 
        layer.name === "total_assets" ||
        layer.name.includes("panchayat_boundaries")
    );

    const isTrendLayerActive = currentLayer?.some(layer => 
        layer.name === "trend_g" ||
        layer.name.includes("mws_layers")
    );
