...
const Map = forwardRef(
  (
    ...
    const LayersArray = [
      ...
      {
        LayerRef: useRef(null),
        name: "Hydrological Boundries",
        isRaster: false,
      },
      { LayerRef: useRef(null), name: "CLART", isRaster: true },
      {
        LayerRef: useRef(null),
        name: "Hydrological Variables",
        isRaster: false,
      },
      ...
      {
        LayerRef: useRef(null),
        name: "Change Detection Afforestation",
        isRaster: true,
      },
      {
        LayerRef: useRef(null),
        name: "Change Detection Deforestation",
        isRaster: true,
      },
      {
        LayerRef: useRef(null),
        name: "Change Detection Degradation",
        isRaster: true,
      },
      {
        LayerRef: useRef(null),
        name: "Change Detection Urbanization",
        isRaster: true,
      },
      {
        LayerRef: useRef(null),
        name: "Change Detection Crop-Intensity",
        isRaster: true,
      },
      {
        LayerRef: useRef(null),
        name: "Change Detection Restoration",
        isRaster: true,
      },
      ...
      {
        LayerRef: useRef(null),
        name: "Fortnight Hydrological Variables",
        isRaster: false,
      },
        ...
        await fetch(AdminURl)
          ...
          .then((Geojson) => {
            if (
              ...
              console.log(
                "Invalid GeoJSON structure for: ",
                data.block.toLowerCase(),
              );
      ...
      try {
        ...
        data.forEach((item) => {
          if (
            ...
            item.district[0].blocks &&
            item.district[0].blocks.length > 0
          ) {
            let tempDS = {
              state: item.label,
    ...
    const getDistrictData = async () => {
      ...
      try {
        ...
        let temp_blocks = state.district
          .map((item) => {
            return {
              district: item.label,
              block:
    ...
    const getBlockData = async () => {
      ...
      try {
        ...
        const temp_blocks = district.blocks
          .map((item) => {
      ...
      try {
        ...
        adminLayer.setStyle(function (feature) {
          ...
          if (isHighlighted) {
            return new Style({
              ...
              fill: new Fill({ color: "rgba(0, 200, 0, 0.8)" }),
            });
          }
        ...
        if (DroughtLayer) {
          DroughtLayer.setStyle(function (feature) {
            ...
            years.map((item) => {
              let tempDro =
                (feature.values_["drlb_" + item].match(/2/g) || []).length +
                (feature.values_["drlb_" + item].match(/3/g) || []).length;
              if (tempDro >= 5) {
                avg_Res++;
              }
            });
        ...
        let ccdLayer = await getVectorLayers("ccd", layerName, true, true);
        ...
        // Debug log
        console.log(
          "Requesting Tree Overall layer:",
          "tree_overall_ch:" + treeOverallLayerName,
        );
        ...
        if (CroppingIntensityLayer) {
          CroppingIntensityLayer.setStyle(function (feature) {
            ...
            let avg_crp_inten =
              (values.cropping_intensity_2017 +
                values.cropping_intensity_2018 +
                values.cropping_intensity_2019 +
                values.cropping_intensity_2020 +
                values.cropping_intensity_2021 +
                values.cropping_intensity_2022 +
                values.cropping_intensity_2023) /
              7;
        ...
        if (TerrainVectorLayer) {
          TerrainVectorLayer.setStyle(function (feature) {
            ...
            return new Style({
              fill: new Fill({
                color:
                  terrainClusterColors[feature.values_.terrainClu] || "#000000",
              }),
            });
          });
        ...
        if (AquiferLayer) {
          AquiferLayer.setStyle(function (feature) {
            ...
            if (feature.values_.Principal_ === "Alluvium") {
              ...
              feature.values_.Principal_ === "Banded Gneissic Complex"
            ) {
              return new Style({
                fill: new Fill({
                  color: "#ffe1b5",
        ...
        if (
          LayersArray[0].LayerRef.current &&
          !currentLayers.includes("Demographics")
        ) {
    ...
    const handleLayerToggle = (name, layerRef) => {
      ...
      // TOGGLE ON: Add the layer if it's not active
        ...
        if (name === "NREGA") {
          ...
          if (LayersArray[6].LayerRef.current) {
            const nregaVectorSource =
              LayersArray[6].LayerRef.current.getSource();
        ...
        // Special cases for hydrology layers (they're mutually exclusive)
          ...
          name === "Hydrological Boundries" &&
          currentLayers.includes("Hydrological Variables")
        ) {
          tempLayer = tempLayer.filter(
            (item) => item !== "Hydrological Variables",
          );
          ...
          name === "Hydrological Variables" &&
          currentLayers.includes("Hydrological Boundries")
        ) {
          tempLayer = tempLayer.filter(
            (item) => item !== "Hydrological Boundries",
          );
      ...
      if (
        !handlingExternalToggle.current &&
        toggleLayer &&
        typeof toggleLayer === "function"
      ) {
        const layerName = name.toLowerCase().replace(/\s+/g, "_");
        console.log(
          `Map is notifying parent that ${layerName} is now ${tempLayer.includes(name) ? "ON" : "OFF"}`,
        );
    ...
    useEffect(() => {
      const fetchLulc = async () => {
        ...
        let LulcLayer = await getImageLayers(
          "LULC_level_1",
          `LULC_${lulcYear1.value}_${district.label
            .toLowerCase()
            .replace(/\s*\(\s*/g, "_")
            .replace(/\s*\)\s*/g, "")
    ...
    useEffect(() => {
      const fetchLulc = async () => {
        ...
        let LulcLayer = await getImageLayers(
          "LULC_level_2",
          `LULC_${lulcYear2.value}_${district.label
            .toLowerCase()
            .replace(/\s*\(\s*/g, "_")
            .replace(/\s*\)\s*/g, "")
            .replace(/\s+/g, "_")}_${block.label
    ...
    useEffect(() => {
      const fetchLulc = async () => {
        ...
        let LulcLayer = await getImageLayers(
          "LULC_level_3",
          `LULC_${lulcYear3.value}_${district.label
            .toLowerCase()
            .replace(/\s*\(\s*/g, "_")
            .replace(/\s*\)\s*/g, "")
            .replace(/\s+/g, "_")}_${block.label
          ...
          <div className="absolute bottom-24 right-6 bg-white p-2 rounded-lg shadow-lg text-sm max-w-xs">
            ...
            <ul className="text-gray-600 text-xs mt-1">
              {Object.entries(layerErrors).map(([layer, error]) => (
                <li key={layer}>
                  {layer}: {error ? error.substring(0, 50) : "Unknown error"}
                </li>
              ))}
            </ul>
          </div>
