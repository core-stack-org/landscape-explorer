...
export const GEOLIBRE_APPLICATION_VERSION = "2.1.0";
...
const tehsilBoundsCache = new Map();
...
export const geoJsonBounds = (featureCollection) => {
  ...
  if (!validBounds(bounds)) {
    throw new Error("The tehsil boundary response did not contain a valid extent.");
  }

  return bounds;
};

export const fetchTehsilBounds = async ({
  district,
  tehsil,
  geoserverUrl = process.env.REACT_APP_GEOSERVER_URL || DEFAULT_GEOSERVER_URL,
}) => {
  if (!district || !tehsil) {
    throw new Error("Select a district and tehsil before finding its extent.");
  }

  const baseUrl = normalizeBaseUrl(geoserverUrl);
  const layerName = `${formatGeoServerName(district)}_${formatGeoServerName(
    tehsil
  )}`;
  const cacheKey = `${baseUrl}panchayat_boundaries:${layerName}`;
  if (tehsilBoundsCache.has(cacheKey)) {
    return tehsilBoundsCache.get(cacheKey);
  }
  ...
  if (!response.ok) {
    throw new Error(`Tehsil boundary request failed (${response.status}).`);
  }

  const bounds = geoJsonBounds(await response.json());
  tehsilBoundsCache.set(cacheKey, bounds);
  return bounds;
};
...
export const mapViewFromBounds = (
  bounds,
  { width = 1200, height = 800, padding = 72 } = {}
) => {
  if (!validBounds(bounds)) {
  ...
  return {
    ...
    zoom: Math.max(3, Math.min(16, Math.floor(zoom * 10) / 10)),
    bearing: 0,
    pitch: 0,
...
const normalizedMapView = (mapView) => {
  const center = Array.isArray(mapView?.center)
    ? mapView.center
  ...
  const normalized = {
    ...
    zoom: Number.isFinite(mapView?.zoom)
      ? mapView.zoom
      : DEFAULT_MAP_VIEW.zoom,
    bearing: Number.isFinite(mapView?.bearing)
      ? mapView.bearing
      : DEFAULT_MAP_VIEW.bearing,
    pitch: Number.isFinite(mapView?.pitch)
      ? mapView.pitch
      : DEFAULT_MAP_VIEW.pitch,
  };

  if (validBounds(mapView?.bbox)) {
    normalized.bbox = mapView.bbox.map(Number);
  }
...
export const selectedGeoLibreLayerIds = ({ toggledLayers, years }) => {
  ...
  for (const [id, value] of Object.entries(years || {})) {
    const hasYear = Array.isArray(value) ? value.length > 0 : Boolean(value);
    if (hasYear && GEOLIBRE_LAYER_BY_ID[id]) selected.push(id);
  }

  return [...new Set(selected)];
};

export const normalizeGeoLibreLulcSelections = (selections) => {
  const validYears = new Set(GEOLIBRE_LULC_YEARS.map(({ value }) => value));
  return Object.fromEntries(
    Object.keys(DEFAULT_GEOLIBRE_LULC_YEARS).map((layerId) => {
      const value = selections?.[layerId];
      const values = Array.isArray(value) ? value : value ? [value] : [];
      return [
        layerId,
        [...new Set(values.filter((year) => validYears.has(year)))],
      ];
    })
  );
};

const lulcYearLabel = (year) =>
  GEOLIBRE_LULC_YEARS.find((option) => option.value === year)?.label || year;

const projectGroupDefinitions = [
  ...GEOLIBRE_LAYER_GROUPS.slice(0, 1),
  { id: "lulc", label: "Land Use / Land Cover" },
  ...GEOLIBRE_LAYER_GROUPS.slice(1),
];
...
export const buildGeoLibreProject = ({
  ...
  const selectedLulc = normalizeGeoLibreLulcSelections(
    lulcSelections || years || DEFAULT_GEOLIBRE_LULC_YEARS
  );
  const hasLulcSelection = Object.values(selectedLulc).some(
    (selectedYears) => selectedYears.length
  );
  if (!selectedIds.size && !hasLulcSelection) {
  ...
  for (const group of GEOLIBRE_LAYER_GROUPS) {
    for (const catalogLayer of group.layers) {
      const selectedYears = catalogLayer.yearKey
        ? selectedLulc[catalogLayer.yearKey]
        : selectedIds.has(catalogLayer.id)
        ? [undefined]
        : [];

      for (const year of selectedYears) {
        const isLulc = Boolean(catalogLayer.yearKey);
        const layer = {
          ...catalogLayer,
          groupId: isLulc ? "lulc" : group.id,
          groupLabel: isLulc ? "Land Use / Land Cover" : group.label,
        };
        const layerName = layer.layerName({ ...scope, year });
        const id = `corestack-${layer.id}${year ? `-${year}` : ""}`;
        const name = year
          ? `${layer.label} · ${lulcYearLabel(year)}`
          : layer.label;
        const style = layerStyle(layer);
        const opacity = layer.id === "demographics" ? 1 : 0;
        ...
        if (layer.sourceType === "wfs") {
          ...
          const wcsDownloadUrl = buildWcsUrl(
            baseUrl,
            layer.workspace,
            layerName
          );
  ...
  const activeGroups = projectGroupDefinitions.filter((group) =>
    layers.some((layer) => layer.groupId === group.id)
  ).map((group) => ({
    id: group.id,
    name: group.label,
    collapsed: group.id !== "demographic",
    visible: true,
    opacity: 1,
  }));
  ...
  return {
    ...
    metadata: {
      ...
      geolibre: {
        ...
        applicationVersion: GEOLIBRE_APPLICATION_VERSION,
        projectVersion: GEOLIBRE_PROJECT_VERSION,
      },
      cameraContract:
        "The initial center and zoom are derived from the selected tehsil WFS extent.",
      qmlStyleContract:
