...
const fetchStewardListing = async (
  stateId,
  organizationId = null,
  districtId = null,
) => {
...
const fetchDistrictCentroid = async (districtName) => {
  ...
  if (beforeParens && beforeParens !== districtName)
    variants.push(beforeParens);
...
const fetchDprReportStatus = async ({
  organizationId = null,
  stateId = null,
  districtId = null,
} = {}) => {
...
const createPinStyle = (stateName, count) => {
  ...
  const stemX = canvasW / 2,
    stemY1 = chipY + chipHeight,
    stemY2 = stemY1 + pinHeight;
  ctx.beginPath();
...
const DOT_DEFAULT = (status = "in_progress") =>
  new Style({
    image: new CircleStyle({
      radius: 9,
      fill: new Fill({ color: PLAN_STATUS_COLORS[status].fill }),
      stroke: new Stroke({
        color: PLAN_STATUS_COLORS[status].stroke,
        width: 2,
      }),
    }),
  });

const DOT_HOVERED = (status = "in_progress") =>
  new Style({
    image: new CircleStyle({
      radius: 12,
      fill: new Fill({ color: PLAN_STATUS_COLORS[status].fill }),
...
const DOT_SELECTED = (status = "in_progress") =>
  new Style({
    image: new CircleStyle({
      radius: 12,
...
const StatCard = ({ label, value }) => (
  ...
  >
    ...
    <p className="text-xs font-medium mt-1" style={{ color: P.muted }}>
      {label}
    </p>
  </div>
);
...
const selectStyles = {
  ...
  placeholder: (base) => ({
    ...base,
    color: "oklch(62% 0.14 301.924)",
    fontSize: "14px",
  }),
  singleValue: (base) => ({
    ...base,
    color: "oklch(28% 0.18 301.924)",
    fontSize: "14px",
  }),
  option: (base, state) => ({
    ...base,
...
const PlansPage = () => {
  ...
  const fetchProposedBlocks = async () => {
    const res = await fetch(
      `${process.env.REACT_APP_API_URL}/proposed_blocks/`,
      {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
  ...
  useEffect(() => {
    const ctx =
      ...
      JSON.parse(sessionStorage.getItem("plansReturnContext") || "null");
    if (!ctx?.stateId) return;
    sessionStorage.removeItem("plansReturnContext");
    ...
    const features = stateData
      ...
      .map((s) => {
        ...
        if (s.centroid?.lat && s.centroid?.lon) {
          ...
          mode === "stewards" &&
          metaStatsRef.current?.state_breakdown
        ) {
          const match = metaStatsRef.current.state_breakdown.find(
            (p) => p.state_name === s.state_name,
          );
          if (match?.centroid)
            coords = [match.centroid.lon, match.centroid.lat];
        }
        ...
        feature.setStyle(
          createPinStyle(s.state_name, s.steward_count ?? s.total_plans),
        );
        return feature;
      })
      .filter(Boolean);
  ...
  useEffect(() => {
    if (viewMode === "plans" && metaStats && isStateView)
      addStateBubbles(metaStats, "plans");
    if (viewMode === "stewards" && stewardStats && isStateView)
      addStateBubbles(stewardStats, "stewards");
  ...
  const addPlanDistrictPinsFromBreakdown = async (districtBreakdown) => {
    ...
    const nameToId = Object.fromEntries(
      Object.entries(districtLookupRef.current).map(([id, name]) => [
        name.toLowerCase(),
        Number(id),
      ]),
    );
    ...
    const centroidResults = await Promise.all(
      districtBreakdown.map(async (d) => {
        ...
        const district_id =
          nameToId[d.district_name?.toLowerCase()] ??
          d.district_id ??
          d.district_soi;
  ...
  const addPlanDistrictPins = async (plans) => {
    ...
    plans.forEach((p) => {
      ...
      if (!districtCounts[p.district_soi]) {
        districtCounts[p.district_soi] = {
          count: 0,
          district_id: p.district_soi,
        };
      }
    ...
    const features = centroidResults.filter(Boolean).map((d) => {
      const feature = new Feature({
        geometry: new Point([d.centroid.lon, d.centroid.lat]),
        districtData: d,
      });
  ...
  const handleStatePinClick = async (stateData) => {
    ...
    try {
      if (viewModeRef.current === "plans") {
        ...
        await addPlanDistrictPinsFromBreakdown(
          stateStats.district_breakdown ?? [],
        );
      } else {
        ...
        if (layerName === "planLayer") {
          if (
            ...
            selectedFeatureRef.current.setStyle(
              DOT_DEFAULT(getFeatureStatus(selectedFeatureRef.current)),
            );
          }
      ...
      try {
        if (viewModeRef.current === "plans") {
          ...
          const [stateStats, dprData, trackingData] = await Promise.all([
            ...
            fetchDprReportStatus({
              organizationId: selected?.value ?? null,
              stateId: currentStateRef.current.state_id,
            }),
            fetchStatusTracking({
              organizationId: selected?.value ?? null,
  ...
  return (
    <div className="min-h-screen" style={{ background: P.lighter }}>
      ...
      <div
        className="bg-white rounded-2xl border shadow-xl m-6"
      ...
      >
        <div className="max-w-[1800px] mx-auto px-6 pt-6 pb-4 relative">
          ...
          <p className="text-md mt-2 text-slate-500 text-center">
            ...
            their communities. This dashboard shows the network coverage, the
            sustainability potential and impact. Discover partner organizations
            and the on-ground work of landscape stewards in creating detailed
            project reports for their villages.
          </p>
        </div>
        ...
        <div className="flex flex-col lg:flex-row gap-6 p-4 lg:p-6 max-w-[1800px] mx-auto h-[calc(100vh-220px)]">
          {/* MAP */}
          <div
            className="relative rounded-2xl overflow-hidden shadow-2xl w-full lg:w-[65%] h-[50vh] lg:h-full"
          ...
          >
            ...
            {planDotsVisible && (
              <div
                className="absolute bottom-4 left-4 z-[1000] rounded-xl px-3 py-2.5 flex flex-col gap-1.5"
                style={{
              ...
              >
                ...
                {[
                  {
                    color: PLAN_STATUS_COLORS.in_progress.fill,
                    label: "In Progress",
                  },
                  {
                    color: PLAN_STATUS_COLORS.dpr_completed.fill,
                    label: "DPR Completed",
                  },
                ].map(({ color, label }) => (
          ...
          {/* SIDEBAR */}
          <div
            className={`w-full lg:w-[35%] h-[50vh] lg:h-full flex flex-col pr-1 relative ${
            ...
            {statsLoading ? (
              ...
              <div className="flex flex-col h-full gap-4">
                <div className="flex flex-col gap-4 flex-1 overflow-y-auto pr-1">
                  <div
                    className="rounded-2xl p-5 text-white shadow-lg relative"
                    style={{
                  ...
                  >
                    <button
                      onClick={() => {
                        if (selectedFeatureRef.current) {
                          selectedFeatureRef.current.setStyle(
                            DOT_DEFAULT(
                              getFeatureStatus(selectedFeatureRef.current),
                            ),
                          );
                    ...
                    <p className="text-2xl font-bold tracking-tight pr-8">
                      {selectedPlan.plan}
                    </p>
                    <p
                      className="text-sm mt-1"
                      style={{ color: "oklch(85% 0.08 301.924)" }}
                    >
                  ...
                  >
                    <div className="grid grid-cols-2 gap-3">
                      {[
                        {
                          label: "Organization",
                          value: selectedPlan.organization_name,
                        },
                        { label: "Project", value: selectedPlan.project_name },
                        {
                          label: "Gram Panchayat",
                          value: selectedPlan.gram_panchayat,
                        },
                      ].map(({ label, value }) => (
                        <div
                          key={label}
                  ...
                  >
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      {[
                        ...
                        {
                          label: "DPR Completed",
                          active: selectedPlan.is_dpr_reviewed,
                        },
                      ].map(({ label, active }) => (
                        <div
                          key={label}
                        ...
                        >
                          ...
                          <div>
                            ...
                            <p
                              className="text-xs font-semibold"
                              style={{
                                color: active ? "oklch(50% 0.18 145)" : P.muted,
                              }}
                            >
                              {active ? "Done" : "Pending"}
                            </p>
                ...
                <button
                  onClick={() => {
                    ...
                    navigate(
                      `/landscape-stewardship/plan-view?id=${selectedPlan.id}&completed=${!!selectedPlan.is_completed}&dpr_reviewed=${!!selectedPlan.is_dpr_reviewed}&dpr_generated=${!!selectedPlan.is_dpr_generated}&dpr_approved=${!!selectedPlan.is_dpr_approved}`,
                      {
                        state: {
                          plan: {
                            ...selectedPlan,
                          ...
                          returnContext: {
                            stateId: currentStateRef.current?.state_id ?? null,
                            stateName:
                              currentStateRef.current?.state_name ?? null,
                          },
                        },
                      },
                    );
                  ...
                  style={{
                    ...
                    cursor: selectedPlan.is_dpr_reviewed
                      ? "pointer"
                      : "not-allowed",
                  }}
                >
                      ...
                      onClick={async () => {
                        ...
                        try {
                          if (!isStateView && currentStateRef.current) {
                            ...
                            if (mode === "plans") {
                              const [stateStats, dprData, trackingData] =
                                await Promise.all([
                                  fetchMetaStats(
                                    orgRef.current?.value ?? null,
                                    currentStateRef.current.state_id,
                              ...
                              statePlansRef.current = [];
                              const [stewardData, stateMetaStats, dprData] =
                                await Promise.all([
                                  fetchStewardStats(
                                    orgRef.current?.value ?? null,
                                    currentStateRef.current.state_id,
                              ...
                              setMetaStats(stateMetaStats);
                              setDprStatus(dprData);
                              await addDistrictPins(
                                stewardData.district_level ?? [],
                              );
                            }
                          } else {
                            // ── Global state view ─────────────────────────
                            if (mode === "plans") {
                              const [data, dprData, trackingData] =
                                await Promise.all([
                                  fetchMetaStats(orgRef.current?.value ?? null),
                                  fetchDprReportStatus({
                                    organizationId:
                              ...
                              const [stewardData, plansMeta, dprData] =
                                await Promise.all([
                                  fetchStewardStats(
                                    orgRef.current?.value ?? null,
                                  ),
                    ...
                    >
                      ...
                      <p className="text-5xl font-bold tracking-tight">
                        {isStateView
                          ...
                          : (
                              stewardStats?.state_level?.[0]?.steward_count ?? 0
                            ).toLocaleString()}
                      </p>
                      <div
                        className="flex gap-4 mt-3 pt-3"
                        style={{
                          borderTop: "1px solid oklch(70% 0.12 301.924)",
                        }}
                      >
                        <div>
                          <p
                          ...
                          <p className="text-lg font-semibold">
                            {stewards?.gender_breakdown?.female}
                          </p>
                        </div>
                        <div
                          className="w-px"
                          style={{ background: "oklch(70% 0.12 301.924)" }}
                        />
                        <div>
                          <p
                            className="text-xs"
                          ...
                          <p className="text-lg font-semibold">
                            {stewards?.gender_breakdown?.male}
                          </p>
                        </div>
                      </div>
                    </div>
                    ...
                    >
                      ...
                      <div className="grid grid-cols-2 gap-3 mb-3">
                        ...
                        >
                          ...
                          >
                            {(
                              dprStatus?.breakdown?.SUBMITTED ?? 0
                            ).toLocaleString()}
                          </p>
                        </div>
                      </div>
                        ...
                        <div className="flex flex-col gap-2">
                          ...
                          >
                            ...
                            <p
                              ...
                              style={{ color: P.base }}
                            >
                              {metaStats?.demand_overview?.community_demands}
                            </p>
                          </div>
                          ...
                          >
                            ...
                            <p
                              ...
                              style={{ color: P.base }}
                            >
                              {metaStats?.demand_overview?.individual_demands}
                            </p>
                          </div>
                        </div>
                      </div>
                          ...
                          {stewardListing.map((s, i) => (
                            <div key={i} className="flex flex-col gap-1">
                              <div
                                ...
                                style={{
                                  background:
                                    selectedSteward === s ? P.light : P.lighter,
                              ...
                              >
                                ...
                                >
                                  ...
                                  <p
                                    ...
                                    style={{ color: P.muted }}
                                  >
                                    {s.plan_count} plans · {s.completed_count}{" "}
                                    completed
                                  </p>
                                </div>
                                <button
                                  onClick={() =>
                                    setStewardModalPlan({
                                      ...
                                      organization:
                                        s.organization?.id ??
                                        getStewardOrgId(s.facilitator_name),
                                    })
                                  }
                    ...
                    >
                      ...
                      <div className="grid grid-cols-3 gap-3">
                        {[
                          ...
                          {
                            label: "Districts",
                            value: commons.active_districts,
                          },
                      ...
                      >
                        ...
                        >
                          ...
                          >
                            ...
                            >
                              {(
                                dprStatus?.breakdown?.APPROVED ?? 0
                              ).toLocaleString()}
                            </p>
                          </div>
                        </div>
                    ...
                    >
                      ...
                      >
                        <div className="flex items-center justify-between mb-3">
                          ...
                          >
                            {(
                              statusTracking?.totals?.SUBMITTED?.demands ?? 0
                            ).toLocaleString()}
                          </p>
                        </div>
                        <div
