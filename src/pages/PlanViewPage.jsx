...
const DataRow = ({ label, value }) => (
  <div className="flex justify-between items-start py-2"
    ...
    <p className="text-xs font-semibold w-2/5" style={{ color: P.muted }}>{label}</p>
    <p className="text-xs text-right w-3/5 font-medium" style={{ color: P.text }}>
      {value ?? "--"}
    </p>
  </div>
);
    ...
    <div className="bg-white rounded-2xl p-5 shadow-sm"
      ...
      <div className="grid grid-cols-2 gap-x-6">
        ...
        <div>
          ...
          <DataRow label="Past Work" value={formatUnderscoreText(s.nrega_past_work)}/>
    ...
    const tryZoom = () => {
      ...
      if ([...rF, ...iF].length > 0) {
        ...
        const combined = extents.reduce((acc, curr) => [
          ...
          Math.max(acc[2], curr[2]), Math.max(acc[3], curr[3]),
        ]);
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    {[
                      ...
                      <div key={label} className="text-center rounded-xl p-2"
                        style={{ background: P.lighter, border: `1px solid ${P.border}` }}>
                        <p className="text-lg font-bold" style={{ color: P.base }}>{value ?? "--"}</p>
