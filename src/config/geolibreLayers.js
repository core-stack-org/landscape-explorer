...
export const LATEST_GEOLIBRE_LULC_YEAR = "24_25";

export const DEFAULT_GEOLIBRE_LULC_YEARS = {
  lulc_level_1: [],
  lulc_level_2: [],
  lulc_level_3: [LATEST_GEOLIBRE_LULC_YEAR],
};
...
export const getDefaultGeoLibreLayerIds = () =>
  GEOLIBRE_LAYERS.filter(
    (layer) => layer.sourceType === "wfs" && !layer.yearKey
  ).map((layer) => layer.id);

export const getGeoLibreNonLulcGroups = () =>
  GEOLIBRE_LAYER_GROUPS.map((group) => ({
    ...group,
    layers: group.layers.filter((layer) => !layer.yearKey),
